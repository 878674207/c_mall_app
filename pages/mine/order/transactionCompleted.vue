<template>
  <view class="creaBox">
    <view class="shopbox" v-for="(item, index) in dataList.orderItemList" :key="index">
      ...
      <view class="goodcomm flex-spb" v-for="(itemTwo, index) in list" :key="index" v-show="moreFlag">
        <p>{{ itemTwo.name }}</p>
        <p v-show="itemTwo.name == '实付款'" class="payment">
      ...
      <view class="goodcomm flex-spb" v-for="(itemTwo, index) in listone" :key="index" v-show="!moreFlag">
        <p>{{ itemTwo.name }}</p>
      ...
      <view class="goodcomm flex-spb" v-for="(itemTwo, index) in listTwo" :key="index" v-show="!moreFlag">
        <p>{{ itemTwo.name }}</p>
        <view v-show="itemTwo.name == '收货信息'" class="goodcommBox">
          <view>{{ dataList.orderAddress.receiverName }}
            {{ dataList.orderAddress.receiverPhone }}
    ...
    <view class="butbox flex-end align-c">
      <view class="cancelOrder">更多</view>
      <view class="btnBox">
        <button class="oederOne" >申请售后</button>
        <button class="oederOne" >物流信息</button>
        <button class="oederTwo" @click="buyNow">再买一单</button>
      </view>
    </view>
    <!-- 弹框 -->
...
export default {
  ...
  methods: {
    ...
    // 点击再买一单
    purchaseAgain() {
      purchaseAgain(this.dataList.id).then((res) => {
        console.log(res)
      })
    },
    // 弹出地址控制函数
    ...
    buyNow() {
      uni.navigateTo({
        url: `/pages/productList/creadOrder?id=${this.dataList.id}`,
      })
    },
    // 点击店铺跳转
    searchActiveshop() {
